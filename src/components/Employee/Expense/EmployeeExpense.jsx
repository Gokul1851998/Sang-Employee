...
import { getDeleteExpense, getExpenseSummary } from "../../../api/ApiCall";
...
export default function EmployeeExpense({id}) {
  ...
  const fetchData = async () => {
    ...
    setDetails(false)
    handleOpen()
    const response = await getExpenseSummary({iUser})
    handleClose()
    if(response.Status === "Success"){
  ...
  >
      {details ? (
        <EmployeeExpenseDetails handleNavigate={handleNavigate} data={dataId} />
      ) : (
        <>
